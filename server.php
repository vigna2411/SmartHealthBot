...
if(isset($_POST['book'])){
  $name1=mysqli_real_escape_string($db,$_POST['uname']);
  $mail1=mysqli_real_escape_string($db,$_POST['mail1']);
  $phone1=mysqli_real_escape_string($db,$_POST['phno1']);
  $hospital = mysqli_real_escape_string($db,$_POST['grouplabel_name']);

  $date1=mysqli_real_escape_string($db,$_POST['date']);
  $time1=mysqli_real_escape_string($db,$_POST['time']);
  
  $sql="INSERT INTO appointment VALUES('$name1','$mail1','$phone1','$hospital','$date1','$time1')";
  mysqli_query($db,$sql);
 
}
if(isset($_POST['my'])){
  
  $sql=mysqli_query($db,"SELECT * FROM appointment WHERE email='$email'");
  
  if ($sql->num_rows > 0) {
    // output data of each row
    while($row = $sql->fetch_assoc()) {
    echo "<tr><td>" . $row["name"]. "</td><td>" . $row["email"] . "</td><td>"
    . $row["phoneno"]. "</td><td>" . $row["date"] . "</td><td>" . $row["time"] . "</td></tr>";
    }
    echo "</table>";
    } else { echo "0 results"; }
  }
